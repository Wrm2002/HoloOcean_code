...
from holoocean.spaces import SensorSpace
...
class HoloOceanEnvironment:
    ...
    def observation_space(self):
        """Gives the observation space for all agents in the environment.

        Returns a dictionary mapping agent names to their observation spaces.
        Each agent's observation space is itself a dictionary mapping sensor
        names to :class:`~holoocean.spaces.SensorSpace` describing the shape
        and dtype of each sensor's data.

        Returns:
            dict: {agent_name: {sensor_name: SensorSpace}}
        """
        obs_space = {}
        for agent_name, agent in self.agents.items():
            agent_obs = {}
            for sensor_name, sensor in agent.sensors.items():
                agent_obs[sensor_name] = SensorSpace(
                    shape=sensor.data_shape,
                    dtype=sensor.dtype
                )
            obs_space[agent_name] = agent_obs
        return obs_space
    ...
    def seed(self, seed=None):
        ...
        Returns:
            list: [seed] in the format expected by Gymnasium.
        """
        if seed is not None:
            np.random.seed(seed)
            random.seed(seed)
    ...
    def render(self, mode='human'):
        ...
        Note: HoloOcean renders through Unreal Engine's viewport.
        This method toggles viewport rendering on/off.

        Args:
            mode (str): Rendering mode. Only 'human' is supported.
                If mode is 'human', enables viewport rendering.
                If mode is 'rgb_array', returns None (not yet supported).

        Returns:
            None: Viewport rendering is handled by UE5 directly.
        """
        if mode == 'human':
