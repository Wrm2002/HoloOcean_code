...
def make_preview(samples: list[Sample], predictions: dict[str, int], out_path: Path, max_items: int = 16) -> None:
    ...
    for idx, sample in enumerate(selected):
        ...
        with Image.open(sample.sonar_path) as image:
            ...
            image.thumbnail((thumb, thumb - 24), RESAMPLE_BILINEAR)
...
def run_training(
    ...
    *,
    ...
    dataset_dir = data.expanduser().resolve()
    out_dir = out.expanduser().resolve()
    out_dir.mkdir(parents=True, exist_ok=True)

    samples = load_samples(dataset_dir)
    x, y = load_features(samples, image_size)
    valid = y >= 0
    x = x[valid]
    y = y[valid]
    ...
    test_count = max(1, int(round(len(samples) * test_ratio))) if len(samples) > 1 else 0
    ...
    if len(classes) == 1:
        ...
        w, b, train_loss, train_acc = train_linear_softmax(
            x_norm[train_idx], y[train_idx], classes, epochs, lr, seed
        )
