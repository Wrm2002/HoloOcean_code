...
DEFAULT_MANIFEST = Path("wrm_projects/02_bigworld_terrain_generation/outputs/generated_terrain_4k_windows_20260609/terrain_tiles_manifest.csv")
DEFAULT_OUT = Path("wrm_projects/05_validation_outputs/bigworld4k_scene_overview_20260610/bigworld4k_scene_overview.png")
...
def render_overview(root: Path, manifest: Path, out_path: Path) -> Path:
    rows = read_manifest(manifest)
    previews = []
    for row in rows:
        raw_preview = Path(row["preview_pgm"].replace("\\", "/"))
        preview = raw_preview if raw_preview.is_absolute() else root / raw_preview
...
def main() -> None:
    ...
    print(render_overview(root, manifest, out_path))
