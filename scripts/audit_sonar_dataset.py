...
def make_preview(frames: list[FrameInfo], out_path: Path, max_frames: int) -> None:
    ...
    for idx, frame in enumerate(selected):
        ...
        with Image.open(frame.sonar_path) as image:
            ...
            image.thumbnail((tile, tile - 28), Image.Resampling.BILINEAR)
...
def main() -> None:
    ...
    make_preview(frames, preview, args.preview_frames)
    print(f"report: {report}")
    print(f"preview: {preview if preview.exists() else 'not written'}")
